...
bool NativeRenderTarget::draw_world_geometry(const WorldFrame& frame,
  ...
  NativeRasterDrawableMetrics& metrics = raster_metrics_.back();
  metrics.stable_id = drawable.stable_id;
  metrics.object_id = stable_hash32(drawable.stable_id);
  if (++raster_stamp_ == 0) {
    std::fill(pixel_stamps_.begin(), pixel_stamps_.end(), 0);
    raster_stamp_ = 1;
  }
  ...
  const auto shade = [&material, &texture, texture_database, texture_salt, shader_salt,
                      material_salt](std::uint32_t salt, float u, float v) noexcept {
  ...
  ++geometry_calls_;
  const auto write_projected = [this, &material, &shade, &metrics](ScreenPoint screen,
                                                                     std::uint32_t salt,
                                                                     float u, float v) noexcept {
    if (screen.x >= width_ || screen.y >= height_ || !std::isfinite(screen.depth)) return false;
    ...
    if (!metrics.screen_bbox_valid) {
      metrics.bbox_min_x = metrics.bbox_max_x = screen.x;
      metrics.bbox_min_y = metrics.bbox_max_y = screen.y;
      metrics.screen_bbox_valid = true;
    } else {
      metrics.bbox_min_x = std::min(metrics.bbox_min_x, screen.x);
      metrics.bbox_min_y = std::min(metrics.bbox_min_y, screen.y);
      metrics.bbox_max_x = std::max(metrics.bbox_max_x, screen.x);
      metrics.bbox_max_y = std::max(metrics.bbox_max_y, screen.y);
    }
  ...
  std::vector<std::uint8_t> projected_valid(decoded.vertices.size(), 0);
  for (std::uint32_t i = 0; i < decoded.vertices.size(); ++i) {
    ...
    ScreenPoint screen{};
    const bool in_view = project(world, screen);
    const bool valid = in_view || project_clamped(world, screen);
    if (valid) {
      projected_vertices[i] = screen;
      projected_valid[i] = 1;
      ++metrics.projected_vertices;
    }
  }
  ...
  const auto raster_clipped_triangle = [&](const ClipVertex& clip_a,
    ...
    for (std::uint32_t y = min_y; y <= max_y; ++y) {
      for (std::uint32_t x = min_x; x <= max_x; ++x) {
        ...
        (void)write_projected(pixel, drawable.asset ^ ordinal ^ salt, u, v);
      }
    }
  };

  // Clip in homogeneous-normalized coordinates before converting to integer
  // pixels.  The previous clamp-only fallback turned off-screen retail
  // triangles into full-viewport overdraw and made the exact 1800-tick run
  ...
  const auto raster_triangle = [&](std::uint32_t ia, std::uint32_t ib,
    ...
    if (!projected_valid[ia] || !projected_valid[ib] || !projected_valid[ic]) {
      ++metrics.clip_rejected_triangles;
      return true;
    }
    const ScreenPoint& a = projected_vertices[ia];
    const ScreenPoint& b = projected_vertices[ib];
    const ScreenPoint& c = projected_vertices[ic];
    ...
    for (std::size_t index = 1; index + 1u < polygon_count; ++index) {
      raster_clipped_triangle(polygon[0], polygon[index], polygon[index + 1u],
