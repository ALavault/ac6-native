...
#define PPC_LWARX(ctx, address) AC6_PPC_LWARX(ctx, address)
#define PPC_STWCX(ctx, address, value) AC6_PPC_STWCX(ctx, address, value)
#define PPC_LDARX(ctx, address) AC6_PPC_LDARX(ctx, address)
#define PPC_STDCX(ctx, address, value) AC6_PPC_STDCX(ctx, address, value)
...
// skipped entirely unless the process-lifetime fast flag was enabled during
// guest entry; the hook itself remains the final guest-mapping guard.
