...
void qualify(const VulkanNormalDrawResult &normal,
  ...
  if (normal.width != 640U || normal.height != 360U ||
      normal.resolved_rgba8.size() != kReachedNormalLinearBytes ||
...
void materialize_reached_normal_edram(const VulkanNormalDrawResult &normal,
                                      Buffer &edram) {
  if (normal.width != kNormalWidth || normal.height != kNormalHeight ||
      normal.resolved_rgba8.size() != kReachedNormalLinearBytes ||
      edram.mapping == nullptr || edram.size != kEdramBytes) {
    throw RuntimeTrap("reached EDRAM buffer is incomplete");
  }
  materialize_reached_normal_rgba8_edram(
      normal.resolved_rgba8,
      std::span<std::byte>(static_cast<std::byte *>(edram.mapping),
                           static_cast<std::size_t>(edram.size)));
}
...
VulkanNeutralResolveResult execute_vulkan_neutral_resolve(
  ...
  try {
    ...
    materialize_reached_normal_edram(normal, edram);
    ...
    const auto certificate = certify_reached_copy_runtime(
        ...
        std::span<const std::byte>(static_cast<const std::byte *>(edram.mapping),
                                   static_cast<std::size_t>(edram.size)));
