...
0. **r480 — piste A (plan approuvé) : une pression `space` unique ~6s après le boot (dernière tentative bornée autorisée par l'utilisateur après r479) n'atteint TOUJOURS PAS les trois nuanceurs cibles de r478. Réutilisé la mécanique `xdotool keydown/keyup` de `tools/ac6-oracle-run.py` (le moteur que `run_gate.py` charge dynamiquement). **Nouveau fait notable** : le boot de ce lancement direct a pris plus de 700s réels (CPU actif en continu, pas gelé) avant l'arrêt manuel — un ordre de grandeur au-delà de toute route `run_gate.py` normale, confirmant sans l'expliquer la divergence de cadencement déjà notée par r479. `xdotool windowclose` a produit un arrêt propre réel (~20s), mais AUCUN fichier `.xpso`/`.xsh` persistant n'a été produit malgré cela — seulement les mêmes 4 nuanceurs vertex que le lancement zéro-entrée de r479. **Recommandation explicite : mettre Piste A en pause ici** — 5 cycles consécutifs (r476-r480) de rendements décroissants sur ce sous-problème précis. Aucune source touchée, profil natif non affecté.**
   Voir
   `reports/ac6-retail-native-codegen-gate2-r480-single-space-confirm-still-misses-the-three-target-shaders-clean-shutdown-produces-no-persistent-cache-20260909.md`.
   **Nommé pour r481** : Piste A en pause (recommandation explicite,
   décision utilisateur à confirmer). Reste ouvert, indépendant :
   Piste B (committer l'arriéré natif stabilisé r433-r479) et Piste C
   (garde-fou HUD) du plan approuvé — toutes deux prêtes à démarrer
   sans dépendre d'une résolution de Piste A.

1. **r479 — piste A (plan approuvé) : hypothèse « lancement direct sans aucune entrée » (nommée par r478) TESTÉE et RÉFUTÉE. Contourné `run_gate.py` (couplage `--dump_shaders`/route scellée non résolu ce cycle) en lançant directement `install/ntsc-uj/bin/ac6recomp` avec `--ac6_d5b4_final_white=true --dump_shaders=<dir>`, sous Xvfb, sans aucune entrée synthétique. Résultat : 4 nuanceurs vertex capturés (`0a6d1dd7767fdf27`, `472913f460d4b446`, `bbaada3605b82c5a`, `c049a8c9e556f129`), AUCUN ne correspond aux trois cibles de r478. Le produit oracle-hybride sans entrée atteint un écran plus précoce/différent (logo de démarrage probable) que celui du sondage natif sans entrée — les deux produits divergent dans leur cadencement de démarrage. Aucune source touchée, profil natif non affecté.**
   Voir
   `reports/ac6-retail-native-codegen-gate2-r479-zero-input-direct-launch-does-not-reach-the-target-screen-hypothesis-refuted-20260909.md`.
   **Nommé pour r480** : (1) répéter avec une entrée synthétique
   minimale après quelques secondes ; (2) modifier `run_gate.py` pour
   découpler `--dump_shaders` de la route scellée. Reste ouvert,
   indépendant : Piste B (arriéré natif r433-r478) et Piste C
   (garde-fou HUD) du plan approuvé.

1. **r478 — piste A (plan approuvé) : trace statique de `draw.primitive_type` (ajoutée à la trace de r476) confirme que les 3 rejets vertex restants sont des états `primitive_type` GÉNUINEMENT non capturés, pas un bug de décodage PM4. Interrogation directe du registre à 320 entrées : deux nuanceurs (`09dd1c7cddae1141`, `57b8e5f14b93cff4`) sont déjà présents mais chacun uniquement sous la valeur OPPOSÉE de `rect_strip_expand` demandée par le sondage natif (inversion symétrique nette) ; un troisième (`4dd456c4ea0923c1`) est absent sous QUELQUE modification que ce soit. Lecture de `native_xenos.cpp` : `primitive_type` est extrait uniformément par `payload[N] & 0x3F` sans aucune dépendance au nuanceur lié — aucun mécanisme plausible pour une « inversion » spécifique à ces deux nuanceurs ; motif cohérent avec des nuanceurs UI génériques réutilisés dans des contextes de dessin différents, pas un défaut de traduction. Aucune capture oracle lancée ce cycle (limite explicite de l'utilisateur respectée). Aucune source committée (trace de diagnostic locale uniquement).**
   Voir
   `reports/ac6-retail-native-codegen-gate2-r478-static-trace-confirms-genuinely-uncaptured-primitive-type-states-not-a-decode-bug-20260909.md`.
   **Nommé pour r479** : décision utilisateur requise — les trois
   motifs manquants nécessitent une NOUVELLE route oracle scellée
   capable d'atteindre l'écran précis que le sondage natif zéro-entrée
   traverse (aucune des trois routes existantes n'y donne accès),
   un travail réel qui dépenserait davantage le budget oracle déjà
   largement utilisé (r454-r478) — pas à décider seul. Reste ouvert,
   indépendant : Piste B (committer l'arriéré natif stabilisé
   r433-r478) et Piste C (garde-fou HUD) du plan approuvé.

1. **r477 — piste A (plan approuvé) : l'hypothèse de minutage de r476 (écran de diagnostic/erreur transitoire) est RÉFUTÉE par une preuve directe. Relancé la même route scellée à `--duration 20` (au lieu de 60) : résultat IDENTIQUE (mêmes valeurs de modification pour les deux nuanceurs vertex ciblés, `0A6D1DD7767FDF27→0x1`, `C049A8C9E556F129→0x900000000`), malgré une fenêtre réduite des deux tiers. Établi aussi que les deux AUTRES routes scellées « longstart » (démarrage pur 60s sans entrée, a priori idéales) sont inutilisables avec la chaîne de dump actuelle — elles n'ont pas les marqueurs de capture requis (`campaign-new-game`/`post-campaign-intro-skip`). Relecture de `draw_pinned()` : le désaccord est structurel — `vertex_high` ne dépend QUE de `draw.primitive_type==0x08` (décodé du flux PM4 invité), pas d'une logique de résolution à corriger côté natif ; probablement deux écrans UI différents réutilisant les mêmes nuanceurs vertex avec des types de primitive différents. Aucune source modifiée ce cycle (analyse uniquement).**
   Voir
   `reports/ac6-retail-native-codegen-gate2-r477-duration-hypothesis-refuted-modification-mismatch-is-structural-not-timing-20260909.md`.
   **Nommé pour r478** : (1) décoder `draw.primitive_type` directement
   dans le sondage natif (pas seulement `vertex_high` dérivé) pour
   trancher entre un vrai bug de décodage PM4 natif (correctif possible
   sans oracle) et un écran de jeu génuinement différent (nécessiterait
   une nouvelle route scellée, décision de budget oracle à ne pas
   prendre seul) ; (2) Piste B (committer l'arriéré natif stabilisé
   r433-r477) et Piste C (garde-fou HUD) du plan approuvé, indépendantes.

1. **r476 — piste A du plan approuvé : campagne oracle ciblée tôt-démarrage exécutée intégralement (route scellée `mission01-qualified-96.steps` tronquée à `--duration 60`, chaîne complète rejouée : dump → traduction → fusion). **10/10 traductions réussies, mais ZÉRO nuanceur réellement nouveau** — les 10 correspondent exactement à des entrées déjà présentes dans le registre à 320 entrées (cohérent : la route de vol de r470 traverse elle-même cet écran-titre avant le vol). **Ceci corrige la prémisse de r475/r476** : le trou n'est pas une couverture de nuanceur manquante. Diagnostic de trace ajouté (`AC6_NATIVE_VD_TRACE`, non committé) : les tirages rejetés du sondage natif échouent TOUJOURS côté vertex (`pixel_ok=1` systématique), et 2 des 3 digests vertex rejetés sont EXACTEMENT ceux capturés ce cycle — mais sous la MAUVAISE modification (`0x1`/`0x900000000` capturés vs `0x900000000`/`0x0` demandés par les tirages réels). Le nuanceur EST dans le registre, juste pas sous la bonne paire (nuanceur, modification) pour ce tirage précis. `ctest` 11/11, aucune régression. Toujours NON committé (PAS un blocage qualifié).**
   Voir
