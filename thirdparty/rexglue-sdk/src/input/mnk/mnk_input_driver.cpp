...
X_RESULT MnkInputDriver::GetState(uint32_t user_index, X_INPUT_STATE* out_state) {
  ...
  // Feed the keystroke queue from the button mask.
  //
  // EnqueueKeystroke existed but had no caller anywhere in the tree, so the
  // queue was always empty and GetKeystroke always returned X_ERROR_EMPTY.
  // Games that read confirmation through XamInputGetKeystroke therefore never
  // saw a press, while anything reading GetState worked normally. That is
  // exactly the AC6 P1.3 symptom measured in cycle 421: on the storage device
  // selector the d-pad moves the selection (GetState) but neither A nor B
  // confirms (GetKeystroke).
  //
  // Edges are derived by diffing against the previous mask, which is what the
  // real API reports -- one keydown when a button goes down, one keyup when it
  // goes up, rather than a repeat per poll.
  {
    const uint16_t changed = static_cast<uint16_t>(buttons ^ last_keystroke_buttons_);
    if (changed) {
      static constexpr struct { uint16_t mask; uint16_t vk; } kPadKeys[] = {
          {X_INPUT_GAMEPAD_A, 0x5800},          {X_INPUT_GAMEPAD_B, 0x5801},
          {X_INPUT_GAMEPAD_X, 0x5802},          {X_INPUT_GAMEPAD_Y, 0x5803},
          {X_INPUT_GAMEPAD_RIGHT_SHOULDER, 0x5804}, {X_INPUT_GAMEPAD_LEFT_SHOULDER, 0x5805},
          {X_INPUT_GAMEPAD_START, 0x5814},      {X_INPUT_GAMEPAD_BACK, 0x5815},
          {X_INPUT_GAMEPAD_DPAD_UP, 0x5810},    {X_INPUT_GAMEPAD_DPAD_DOWN, 0x5811},
          {X_INPUT_GAMEPAD_DPAD_LEFT, 0x5812},  {X_INPUT_GAMEPAD_DPAD_RIGHT, 0x5813},
          {X_INPUT_GAMEPAD_LEFT_THUMB, 0x5806}, {X_INPUT_GAMEPAD_RIGHT_THUMB, 0x5807},
      };
      for (const auto& k : kPadKeys) {
        if (changed & k.mask) {
          EnqueueKeystroke(k.vk, (buttons & k.mask) != 0);
        }
      }
      last_keystroke_buttons_ = static_cast<uint16_t>(buttons);
    }
  }
  ...
  // packet_number must change ONLY when the state changes.
  //
  // XInput's contract is that a caller can compare packet_number between polls
  // to learn whether anything moved; an unchanged number means an identical
  // state. This incremented on every call, so every poll advertised a change
  // and the field carried no information. Games that latch new input on a
  // packet change therefore saw a fresh "change" every frame, including frames
  // where the pad was untouched.
  //
  // Compare the full pad payload, not just buttons, so stick and trigger
  // movement still register.
  const bool state_changed =
      buttons != prev_pad_buttons_ || lt != prev_pad_lt_ || rt != prev_pad_rt_ ||
      clamp16(lx) != prev_pad_lx_ || clamp16(ly) != prev_pad_ly_ ||
      clamp16(rx) != prev_pad_rx_ || clamp16(ry) != prev_pad_ry_;
  if (state_changed) {
    packet_number_++;
    prev_pad_buttons_ = buttons;
    prev_pad_lt_ = lt;
    prev_pad_rt_ = rt;
    prev_pad_lx_ = clamp16(lx);
    prev_pad_ly_ = clamp16(ly);
    prev_pad_rx_ = clamp16(rx);
    prev_pad_ry_ = clamp16(ry);
  }
