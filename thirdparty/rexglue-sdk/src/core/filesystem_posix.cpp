...
std::unique_ptr<FileHandle> FileHandle::OpenExisting(const std::filesystem::path& path,
  ...
  // O_RDONLY is zero on POSIX, so access modes cannot be accumulated with
  // bitwise OR. Doing that turns a read+write request into O_WRONLY and makes
  // the first pread fail with EBADF. Select exactly one access mode instead.
