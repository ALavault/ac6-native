...
static void ExceptionHandlerCallback(int signal_number, siginfo_t* signal_info,
  ...
  const char* signal_name = signal_number == SIGILL    ? "SIGILL (illegal instruction)"
                            : signal_number == SIGSEGV ? "SIGSEGV (access violation)"
                                                       : "unknown signal";
#if REX_ARCH_AMD64
  const uint64_t host_pc = uint64_t(mcontext.gregs[REG_RIP]);
#elif REX_ARCH_ARM64
  const uint64_t host_pc = uint64_t(mcontext.pc);
#else
  const uint64_t host_pc = 0;
#endif
  Dl_info module_info{};
  const bool module_found = dladdr(reinterpret_cast<void*>(host_pc), &module_info) != 0;
  const uint64_t module_base = module_found
                                   ? reinterpret_cast<uint64_t>(module_info.dli_fbase)
                                   : 0;
  REXLOG_ERROR(
      "Unhandled {} at host pc {:#018x} (module base {:#018x}, offset {:#x}), faulting address "
      "{:#018x}. No installed handler claimed "
      ...
      signal_name, host_pc, module_base, host_pc - module_base,
      reinterpret_cast<uint64_t>(signal_info->si_addr));
