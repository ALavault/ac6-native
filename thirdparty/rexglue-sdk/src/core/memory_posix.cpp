...
FileMappingHandle CreateFileMappingHandle(const std::filesystem::path& path, size_t length,
  ...
  // The descriptor keeps the shared-memory object alive after unlinking it.
  // Removing the name now makes the mapping crash-safe: if the process exits
  // before CloseFileMappingHandle, the kernel reclaims the object when the
  // last descriptor or mapping is closed instead of leaking it in /dev/shm.
