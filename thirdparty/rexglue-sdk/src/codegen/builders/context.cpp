...
void BuilderContext::emit_function_call(uint32_t address) {
  ...
  // Overlapping analysis can discover the exact callee entry after the
  // caller's edge set was built. Classification still knows that this address
  // is an independently declared function, so emit that exact graph target
  // instead of degrading a valid cross-function branch into a runtime fatal.
  if (auto* targetFn = graph().getFunction(address);
      targetFn != nullptr && !targetFn->name().empty() && !graph().isImport(address)) {
    const auto& name = targetFn->name();
    if (cfg.nonVolatileRegistersAsLocalVariables &&
        (name.find("__rest") == 0 || name.find("__save") == 0)) {
      return;
...
void BuilderContext::emit_conditional_branch(bool not_, std::string_view cond) {
  ...
  const auto graphKind = graph().classifyTarget(target, base, false);
  const bool targetInBlock =
      std::ranges::any_of(fn.blocks(), [target](const auto& block) { return block.contains(target); });

  // An exact declared entry point remains callable even when overlapping
  // analysis also places it in this function's blocks. Block containment is
  // only a fallback for labels that the graph does not identify as a separate
  // function. Otherwise the generated local goto has no label once codegen
  // emits the overlapping entry in its own PPC_FUNC_IMPL.
  auto kind = graphKind;
  if (target == fn.base() ||
      (targetInBlock && graphKind != TargetKind::Function && graphKind != TargetKind::Import)) {
    ...
    case TargetKind::Import:
      ...
      if (const auto* callTarget = findCallTarget(base)) {
        ...
        if (auto* targetFn = graph().getFunction(target);
            targetFn != nullptr && !targetFn->name().empty() && !graph().isImport(target)) {
          println("\tif ({}{}.{}) {{", not_ ? "!" : "", cr(insn.operands[0]), cond);
          println("\t\t{}(ctx, base);", targetFn->name());
          println("\t\treturn;");
          println("\t}}");
          ...
          REXCODEGEN_ERROR(
              "Unresolved conditional branch to 0x{:08X} from 0x{:08X} (no CallTarget)", target,
              base);
