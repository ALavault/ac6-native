...
void SpirvShaderTranslator::ProcessTextureFetchInstruction(
  ...
  if (REXCVAR_GET(ac6_force_white_texture_sample) &&
    ...
    // Liveness control: prove the restriction fires before reading any result
    // from it. Cycle 358 produced output identical to the unrestricted force,
    // which is impossible if this branch is taken -- so it must be shown.
    REXLOG_INFO("[ac6-forcetex] forcing white sample in ps={:016X}",
