...
REXCVAR_DEFINE_UINT32(ac6_skip_texture_base, 0u, "AC6/Render",
                      "Diagnostic: skip every draw binding this guest texture base");
...
bool VulkanCommandProcessor::IssueDraw(xenos::PrimitiveType prim_type, uint32_t index_count,
  ...
  if (REXCVAR_GET(ac6_log_vulkan_draws)) {
    ...
    REXGPU_INFO(
        // ndc_scale needs far more than three decimals: the clip_disable path
        // (draw.cpp) sets it to scale_xy * 2/8192, which is 0.000244 for an
        // unscaled axis. At {:.3f} that prints as a flat 0.000 and reads as a
        // degenerate transform that collapses all geometry. It is not one --
        // cycle 398 drew exactly that false conclusion from this format string.
        "[ac6-draw2] vs={:016X} ps={:016X} vp_off={},{} vp_ext={}x{} z={:.3f}..{:.3f} "
        "ndc_s={:.9g},{:.9g} ndc_o={:.9g},{:.9g} alpha_test={} alpha_func={} tex_bind={}",
        vertex_shader ? vertex_shader->ucode_data_hash() : 0ull,
    ...
    // The two remaining candidates are an empty/absent bound texture and a
    // mistranslated pixel shader. This separates them: a glyph sheet that is
    // absent or zero-sized leaves the geometry running and the text invisible,
    // which is exactly the observed picture. Guest base address, format and
    // dimensions come straight from the fetch constant the binding names.
    if (pixel_shader) {
      for (const auto& binding : pixel_shader->GetTextureBindingsAfterTranslation()) {
        ...
        REXGPU_INFO(
            "[ac6-tex] ps={:016X} fc={} type={} base={:08X} fmt={} dim={} w={} h={}",
            pixel_shader->ucode_data_hash(), uint32_t(binding.fetch_constant),
            uint32_t(fetch.type), uint32_t(fetch.base_address) << 12, uint32_t(fetch.format),
            uint32_t(binding.dimension), uint32_t(fetch.size_2d.width) + 1,
            uint32_t(fetch.size_2d.height) + 1);
        // Remaining descriptor fields. Cycle 362 named the two textures that
        // sample to nothing -- both glyph sheets -- while five others in the
        // same pass, same fmt=20, render correctly. Swizzle and the sampled
        // channel are the leading suspects: a glyph sheet usually carries its
        // information in alpha alone.
        REXGPU_INFO(
            "[ac6-tex2] base={:08X} swizzle={:#05X} num_format={} endian={} tiled={} "
            "pitch={} exp_adjust={} mag={} min={} mip={}",
            uint32_t(fetch.base_address) << 12, uint32_t(fetch.swizzle),
            uint32_t(fetch.num_format), uint32_t(fetch.endianness), uint32_t(fetch.tiled),
            uint32_t(fetch.pitch), int32_t(fetch.exp_adjust), uint32_t(fetch.mag_filter),
            uint32_t(fetch.min_filter), uint32_t(fetch.mip_filter));
        // Mip selection, never measured. If mip_max_level selects a level that
        // was never populated, the sample is zero while every other property
        // stays correct -- which matches these two textures exactly.
        REXGPU_INFO("[ac6-mip] base={:08X} mip_min={} mip_max={} mip_address={:08X}",
                    uint32_t(fetch.base_address) << 12, uint32_t(fetch.mip_min_level),
                    uint32_t(fetch.mip_max_level), uint32_t(fetch.mip_address) << 12);
      }
    }
  }

  // Tie a draw batch to a region of the screen (cycle 385). Skipping every draw
  // that binds a named texture makes the pixels it owns disappear, which is the
  // one thing twenty-seven eliminations never established. Diagnostic only.
  // Toggle the skip on a 2-second cycle so both phases occur inside ONE run.
  // Comparing two separate runs failed (cycle 386): they do not reach the same
  // state, and 66% of pixels differed for reasons unrelated to the skip.
  bool skip_phase = false;
  {
    // Second control (cycle 389): prove the PARAMETER arrived, not just that the
    // probe ran. Without it, "skip fired 0" cannot distinguish an unread cvar
    // from a state never reached.
    static std::atomic<bool> announced{false};
    bool expected = false;
    if (announced.compare_exchange_strong(expected, true)) {
      REXGPU_WARN("[ac6-skip] ac6_skip_texture_base = {:#010x}",
                  uint32_t(REXCVAR_GET(ac6_skip_texture_base)));
    }
  }
  if (REXCVAR_GET(ac6_skip_texture_base)) {
    const uint64_t sec = uint64_t(std::chrono::duration_cast<std::chrono::seconds>(
                                      std::chrono::steady_clock::now().time_since_epoch())
                                      .count());
    skip_phase = (sec % 4) < 2;
  }
  if (pixel_shader && skip_phase) {
    for (const auto& binding : pixel_shader->GetTextureBindingsAfterTranslation()) {
      const xenos::xe_gpu_texture_fetch_t fetch = regs.GetTextureFetch(binding.fetch_constant);
      if ((uint32_t(fetch.base_address) << 12) == uint32_t(REXCVAR_GET(ac6_skip_texture_base))) {
        // Liveness control. A zero pixel delta is unreadable without proof that
        // the skip actually fired -- the omission that made cycle 387's result
        // uninterpretable.
        static std::atomic<uint32_t> skipped{0};
        const uint32_t n = ++skipped;
        if (n <= 4 || (n % 200) == 0) {
          REXGPU_WARN("[ac6-skip] skipped draw #{} binding base={:08X}", n,
                      uint32_t(fetch.base_address) << 12);
        }
        return true;  // pretend the draw succeeded, emit nothing
      }
    }
  }
...
bool VulkanCommandProcessor::IssueCopy() {
  ...
  if (readback_mode == ReadbackResolveMode::kDisabled) {
    ...
    const bool resolved = render_target_cache_->Resolve(*memory_, *shared_memory_, *texture_cache_,
                                                        written_address, written_length);
    // AC6 P1.3. Cycle 366 showed the two glyph sheets are loaded once and never
    // invalidated, so they are most likely produced by a GPU resolve into their
    // range. This logs what each resolve actually writes, flagging those two
    // bases, and logs a global count so silence on them is distinguishable from
    // a resolve path that never runs.
    if (resolved) {
      static std::atomic<uint32_t> n_resolve{0};
      const uint32_t n = ++n_resolve;
      const bool hits_glyph =
          (0x03514000u >= written_address && 0x03514000u < written_address + written_length) ||
          (0x028B7000u >= written_address && 0x028B7000u < written_address + written_length);
      if (hits_glyph) {
        REXGPU_WARN("[ac6-resolve] #{} writes {:08X}+{:X}   <-- COVERS A GLYPH SHEET", n,
                    written_address, written_length);
      } else if (n <= 6 || (n % 300) == 0) {
        REXGPU_INFO("[ac6-resolve] #{} writes {:08X}+{:X}", n, written_address, written_length);
      }
    }
    return resolved;
  }
