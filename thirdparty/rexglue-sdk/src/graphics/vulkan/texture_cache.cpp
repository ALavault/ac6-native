...
VkImageView VulkanTextureCache::GetActiveBindingOrNullImageView(uint32_t fetch_constant_index,
  ...
  // six times.
  {
    ...
    if (image_view != VK_NULL_HANDLE) {
      ...
      if (n <= 24 || (n % 500) == 0) {
        // A null view has exactly three causes here and they call for three
        // different fixes, so say which one rather than only that it happened:
        // no valid binding at all (the texture never became resident), a
        // dimension mismatch between what the shader fetches and what the
        // texture was created as (AreDimensionsCompatible rejects it), or a
        // binding that is present and compatible but whose image view was never
        // created. Counting them together is what let this sit as "samples to
        // zero, cause unknown" across several cycles.
        const char* why = !binding ? "no-valid-binding"
                          : !AreDimensionsCompatible(dimension, binding->key.dimension)
                              ? "dimension-mismatch"
                              : "no-image-view";
        // "no-image-view" is still three distinct faults wearing one name, and
        // they are set in UpdateTextureBindings: the texture pointer is null,
        // the sign the shader asks for is not among the binding's swizzled
        // signs, or the guest format has no host equivalent
        // (format_*.format == VK_FORMAT_UNDEFINED) so the view is never made.
        // The last one is a format-support gap, not a cache miss, and is the
        // only one that would hit a specific pair of textures every frame while
        // everything around them renders.
        if (binding && !std::strcmp(why, "no-image-view")) {
          const HostFormatPair& hf = GetHostFormatPair(binding->key);
          REXGPU_WARN(
              "[ac6-bind] NULL VIEW #{} fc={} base={:08X} fetch_dim={} bound_dim={} why={} "
              "guest_format={} signed={} uses_unsigned={} uses_signed={} host_unsigned_undef={} "
              "host_signed_undef={} texture={} texture_signed={}",
              n, fetch_constant_index, base, uint32_t(dimension), int(binding->key.dimension), why,
              uint32_t(binding->key.format), is_signed ? 1 : 0,
              texture_util::IsAnySignNotSigned(binding->swizzled_signs) ? 1 : 0,
              texture_util::IsAnySignSigned(binding->swizzled_signs) ? 1 : 0,
              hf.format_unsigned.format == VK_FORMAT_UNDEFINED ? 1 : 0,
              hf.format_signed.format == VK_FORMAT_UNDEFINED ? 1 : 0,
              binding->texture ? 1 : 0, binding->texture_signed ? 1 : 0);
        } else {
          REXGPU_WARN("[ac6-bind] NULL VIEW #{} fc={} base={:08X} fetch_dim={} bound_dim={} why={}",
                      n, fetch_constant_index, base, uint32_t(dimension),
                      binding ? int(binding->key.dimension) : -1, why);
        }
      }
    }
  }
      ...
      // seven of the pass's textures are logged so a zero is interpretable.
      {
        ...
        if (is_glyph || is_art) {
          REXGPU_WARN("[ac6-dispatch] base={:08X} groups={}x{}x{} size_blocks={}x{}x{}{}",
                      ...
                      is_glyph ? "   <-- FAILS" : "   <-- WORKS");
          // Addressing is the only mechanism left (cycle 381): this shader is a
          // pure block copy, so the guest offset/pitch constants are what decide
          // where each block is read from.
          REXGPU_WARN(
              "[ac6-loadconst] base={:08X} guest_offset={:08X} guest_pitch_aligned={} "
              "guest_z_stride_rows={} host_offset={:08X} host_pitch={} height_texels={} "
              "tiled3d_endian_scale={:08X}",
              probe_base, load_constants.guest_offset, load_constants.guest_pitch_aligned,
              load_constants.guest_z_stride_block_rows_aligned, load_constants.host_offset,
              load_constants.host_pitch, load_constants.height_texels,
              load_constants.is_tiled_3d_endian_scale);
        }
      }
