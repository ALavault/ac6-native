...
REXCVAR_DEFINE_BOOL(ac6_log_profile_queries, false, "AC6/Kernel",
                    "Log the sign-in and profile queries the guest makes, with caller and result.");
...
// AC6 stops on a modal that Xenia renders as "A gamer profile has not been
// selected. You will be unable to save Game Data. Continue anyway?" -- so the
// guest has asked the runtime something about the signed-in user and did not
// like the answer. This runtime reports a perfectly valid local profile
// (signin_state 1, type local|online, a non-zero XUID and the name "User"), so
// whichever query drives that dialog either is not one of those, or is being
// answered in a way the title rejects.
//
// Fifty cycles went into the button that dismisses the dialog. Nobody asked
// what the dialog is about. This is that question, and it is a control-flow
// one: which profile call does the guest make, from where, and what does it get
// back. Warn level and cvar-gated, so it costs nothing when off.
uint32_t GuestCaller() {
...
void LogProfileQuery(const char* fn, uint32_t user_index, uint64_t value, uint32_t result) {
  if (!REXCVAR_GET(ac6_log_profile_queries)) {
    return;
  }
  REXKRNL_WARN("[ac6-profile] {} user={} value=0x{:016X} result=0x{:08X} caller=0x{:08X}", fn,
               user_index, value, result, GuestCaller());
}
...
ppc_hresult_result_t XamUserGetXUID_entry(ppc_u32_t user_index, ppc_u32_t type_mask,
  ...
  *xuid_ptr = xuid;
  LogProfileQuery("XamUserGetXUID", user_index, xuid, result);
  return result;
}
...
ppc_u32_result_t XamUserGetSigninState_entry(ppc_u32_t user_index) {
  ...
  LogProfileQuery("XamUserGetSigninState", user_index, signin_state, 0);
  return signin_state;
}
...
ppc_hresult_result_t XamUserGetSigninInfo_entry(ppc_u32_t user_index, ppc_u32_t flags,
  ...
  LogProfileQuery("XamUserGetSigninInfo", user_index, user_profile->signin_state(), X_E_SUCCESS);
  return X_E_SUCCESS;
}
...
ppc_u32_result_t XamUserGetName_entry(ppc_u32_t user_index, ppc_pchar_t buffer,
  ...
  LogProfileQuery("XamUserGetName", user_index, 0, X_E_SUCCESS);
  return X_E_SUCCESS;
}
...
ppc_u32_result_t XamUserCheckPrivilege_entry(ppc_u32_t user_index, ppc_u32_t mask,
  ...
  *out_value = 0;
  LogProfileQuery("XamUserCheckPrivilege", user_index, mask, X_ERROR_SUCCESS);
  return X_ERROR_SUCCESS;
}
...
ppc_u32_result_t XamShowSigninUI_entry(ppc_u32_t unk, ppc_u32_t unk_mask) {
  LogProfileQuery("XamShowSigninUI", unk, unk_mask, 0);
