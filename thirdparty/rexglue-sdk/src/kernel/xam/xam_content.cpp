...
REXCVAR_DEFINE_BOOL(ac6_fake_content_item, false, "AC6/Content",
                   "Inject one dummy save into empty content enumerations (P1.3 test only).");
...
ppc_u32_result_t XamContentCreateEnumerator_entry(ppc_u32_t user_index, ppc_u32_t device_id,
  ...
  // P1.3 deciding test (cycle 420): does an empty enumeration cause the freeze?
  //
  // The guest opens the storage device selector, the modal completes with
  // SUCCESS and device_id 1, this enumerator returns SUCCESS with a valid
  // handle and zero items, and the guest then stops calling the kernel
  // altogether and redraws an empty dialog forever. Zero items being the last
  // call does not make it the cause -- the oracle shows empty save slots, so a
  // game with no saves must draw its browser rather than freeze.
  //
  // This settles it by experiment rather than argument: inject one well-formed
  // item and see whether the guest advances. If it advances, the empty
  // enumeration was the cause after all; if it stays frozen, the enumerator is
  // ruled out for good. Behind a cvar and off by default -- it fabricates
  // content that does not exist on disk, so it must never be on for a normal
  // run.
  if (REXCVAR_GET(ac6_fake_content_item) && e->item_count() == 0) {
    auto item = e->AppendItem();
    if (item) {
      std::memset(item, 0, sizeof(*item));
      item->device_id = device_id ? uint32_t(device_id) : uint32_t(DummyDeviceId::HDD);
      item->content_type = XContentType(uint32_t(content_type));
      item->set_display_name(u"AC6 TEST SAVE");
      item->set_file_name("ac6test");
      REXKRNL_WARN("[ac6-fake-content] injected one dummy item for the P1.3 test");
    }
  }
