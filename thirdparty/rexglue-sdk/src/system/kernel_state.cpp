...
void KernelState::CompleteOverlappedEx(uint32_t overlapped_ptr, X_RESULT result,
  ...
  // P1.3 cycle 434: what does the guest's overlapped actually contain?
  //
  // A completion writes result, signals the event, and enqueues an APC only if
  // the completion routine field is non-zero. A structure left as FEFEFEFE fill
  // would read as a non-zero routine whose context resolves to nothing, so no
  // APC is enqueued and the guest's completion handler never runs. That is a
  // candidate for the AC6 device selector never leaving its pending state, but
  // the fields had never been sampled -- only inferred. This logs them.
  REXSYS_INFO("[ovl] {:08X} result={:08X} event={:08X} routine={:08X} context={:08X} len={}",
              overlapped_ptr, result, uint32_t(event_handle),
              XOverlappedGetCompletionRoutine(ptr), XOverlappedGetContext(ptr), length);
  if (event_handle) {
