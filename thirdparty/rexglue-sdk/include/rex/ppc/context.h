...
#define PPC_CALL_INDIRECT_FUNC(x)                                             \
  do {                                                                        \
    const uint32_t _guest_target = static_cast<uint32_t>(x);                  \
    PPCFunc* const _indirect_target = PPC_LOOKUP_FUNC(base, _guest_target);   \
    if (_indirect_target == nullptr) {                                        \
      REXLOG_ERROR(                                                           \
          "[recomp-invalid-indirect] target=0x{:08X} caller=0x{:08X}",       \
          _guest_target, ctx.lr);                                             \
    }                                                                         \
    _indirect_target(ctx, base);                                              \
  } while (false)
