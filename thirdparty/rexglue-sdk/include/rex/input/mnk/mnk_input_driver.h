...
class MnkInputDriver final : public InputDriver,
 ...
 private:
  ...
  // Previous button mask, for deriving keystroke edges in GetState.
  uint16_t last_keystroke_buttons_ = 0;
  // Previous pad payload, so packet_number only advances on a real change.
  uint16_t prev_pad_buttons_ = 0;
  uint8_t prev_pad_lt_ = 0;
  uint8_t prev_pad_rt_ = 0;
  int16_t prev_pad_lx_ = 0;
  int16_t prev_pad_ly_ = 0;
  int16_t prev_pad_rx_ = 0;
  int16_t prev_pad_ry_ = 0;
