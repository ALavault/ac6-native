...
> **Statut d'intégration (main, 2026-08-17).** Proposition de recherche importée
> de `origin/infos` (`1948471c`), analyse statique seule ; elle ne ferme aucune
> gate de `planning/ac6-demo-player/goal-gate-matrix-v1.json` et n'ouvre pas le
> domaine 2 de `ac6-demo-native`.
>
> L'archive `.zip` citée ci-dessus **n'a pas été intégrée** : elle est tronquée
> (14 476 octets, aucun End-of-Central-Directory, dernier flux deflate coupé) et
> `.gitignore:107` exclut `*.zip` de ce dépôt, dont la politique est de ne
> versionner que du texte. Onze tables y étaient déclarées ; dix ont été
> extraites intactes et sont versionnées en clair à côté de ce rapport :
>
> `analysis/ac6-demo-{hud-bridge, radio-namespace-contract,
> debriefing-event-types, debriefing-state-machine, order-dispatch-contract,
> key-service-semantics, debriefing-timeline-fields, return-line-contract,
> key-group-script-summaries, difficulty-durability-factors}.csv`
>
> La onzième, `analysis/ac6-demo-act-start-conditions.csv` (20 214 octets
> annoncés), est **perdue** : son flux est celui que la troncature coupe et sa
> décompression échoue (`invalid distance too far back`). Les conditions de
> départ d'Act citées dans ce rapport ne sont donc appuyées par aucune table
> livrée ; les republier demande de régénérer l'extracteur côté `infos`.
