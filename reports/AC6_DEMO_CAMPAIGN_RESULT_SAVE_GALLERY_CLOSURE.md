...
> **Statut d'intégration (main, 2026-08-17).** Recherche importée de
> `origin/infos`, analyse statique seule ; elle ne ferme aucune gate de
> `planning/ac6-demo-player/goal-gate-matrix-v1.json`.
>
> L'archive `.zip` citée n'a pas été intégrée : elle est **tronquée** (aucun
> End-of-Central-Directory, dernier flux deflate coupé), et `.gitignore:107`
> exclut `*.zip` d'un dépôt dont la politique est de ne versionner que du
> texte. C'est la **troisième** archive consécutive de cette branche dans ce
> cas — après celles de `1948471c` et de ce même lot — ce qui pointe un défaut
> systématique du producteur d'archives, pas un accident.
>
> Les tables décompressables sont versionnées en clair dans le répertoire
> voisin indiqué ci-dessous ; celle que la troncature coupe est **perdue**, et
> les affirmations du rapport qui s'y appuient ne sont donc étayées par aucune
> table livrée tant que l'extracteur n'est pas relancé côté `infos`.
>
> Tables livrées : `analysis/ac6-demo-campaign-result-save-20260817/` — table perdue : `mission_result_layout.csv`.
