// Read-only assertions for the AC6 PAL 0x821DE7D0 configured split.
// @category AC6
...
public class Verify821DE7D0Boundary extends GhidraScript {
    ...
    private void require(long value, String mnemonic, String fragment)
        ...
        assertions++;
        println("AC6_821DE7D0_CONTRACT=" + address + " " + instruction);
    }
    ...
    private void requireNoReferences(long value) {
        ...
        assertions++;
        println("AC6_821DE7D0_NO_REFERENCES=" + address);
    }
    ...
    private void requireReference(long sourceValue, long targetValue) {
        ...
        Reference[] references = currentProgram.getReferenceManager().getReferencesFrom(source);
        for (Reference reference : references) {
            if (reference.getToAddress().equals(target)) {
                assertions++;
                println("AC6_821DE7D0_REFERENCE=" + source + " -> " + target);
                return;
            }
        }
    ...
    private void requireFunctionEntry(long value) {
        ...
        assertions++;
        println("AC6_821DE7D0_FUNCTION_ENTRY=" + function.getEntryPoint());
    }
    ...
    private void requireNoFunctionEntry(long value) {
        ...
        assertions++;
        println("AC6_821DE7D0_NO_FUNCTION_ENTRY=" + address);
    }

    @Override
    public void run() throws Exception {
        // The only ABI entry saves LR/registers and preserves source/destination
        // before 0x821DE7D0 loads the first declaration element.
        require(0x821de7a8L, "mfspr", "r12,LR");
        require(0x821de7acL, "bl", "0x82382ef4");
        require(0x821de7b0L, "stwu", "r1,-0x90(r1)");
        require(0x821de7b4L, "li", "r27,0x0");
        ...
        // 0x821DE7D0 is a straight-line load dependent on r29 from the entry.
        // It has no incoming reference, no prologue, and no function entry.
        require(0x821de7d0L, "lhz", "r11,0x0(r29)");
        requireNoReferences(0x821de7d0L);
        require(0x821de7d4L, "std", "r27,0x58(r1)");
        require(0x821de7d8L, "b", "0x821de800");

        // The declaration scan loops through 12-byte source records until the
        // 0xff sentinel. Preserve the separately configured 0x821DE7E8 target:
        // this audit records its branch but does not classify or remove it.
        require(0x821de7dcL, "cmplw", "cr6,r28,r11");
        require(0x821de7e0L, "bgt", "cr6,0x821de7e8");
        requireReference(0x821de7e0L, 0x821de7e8L);
        ...
        // Allocation metadata and the record-copy loop consume state computed
        // before and across the configured split, then share one epilogue.
        require(0x821de808L, "mulli", "r11,r30,0xc");
        require(0x821de80cL, "addi", "r5,r11,0x38");
        ...
        require(0x821de864L, "lwz", "r9,0x0(r11)");
        require(0x821de868L, "subic.", "r30,r30,0x1");
        require(0x821de86cL, "stw", "r9,0x0(r10)");
        require(0x821de87cL, "addi", "r11,r11,0xc");
        require(0x821de884L, "addi", "r10,r10,0xc");
