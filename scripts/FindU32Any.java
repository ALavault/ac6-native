...
public class FindU32Any extends GhidraScript {
    @Override
    protected void run() throws Exception {
        String[] args = getScriptArgs();
        if (args.length != 1) {
            throw new IllegalArgumentException("usage: FindU32Any <value>");
        }
        ...
        for (MemoryBlock block : currentProgram.getMemory().getBlocks()) {
            ...
            while (current.compareTo(last) <= 0) {
                ...
                long be = ((long)(bytes[0] & 0xff) << 24) |
                          ((long)(bytes[1] & 0xff) << 16) |
                          ((long)(bytes[2] & 0xff) << 8) |
                          (long)(bytes[3] & 0xff);
                if (be == expected) println(current.toString());
