// Read-only byte qualification for the US Mission 01 objective driver.
// The driver is reached through a native dispatch table; this script records
// its .pdata extents and the evaluator it invokes without modifying Ghidra.
...
public class QualifyUsMission01ObjectiveDriver extends GhidraScript {
    ...
    private static final long[] HOOKS = {
        0x82256490L, // native unit/object driver
        0x8226c068L, // objective condition evaluator
    };
    ...
    protected void run() throws Exception {
        ...
        for (int index = 0; index < HOOKS.length; ++index) {
            ...
            println(String.format(
                "AC6_US_MISSION01_OBJECTIVE_HOOK label=%s address=0x%08X "
                + "end_exclusive=0x%08X length=%d extent_sha256=%s "
                + "entry_bytes=%s ghidra_body=%s",
                LABELS[index], hook, hook + length, length, sha256(extentBytes),
                hex(read(hook, 32)), function.getBody()));
        }
        println("AC6_US_MISSION01_OBJECTIVE_PASS hooks=" + HOOKS.length);
    }
}
