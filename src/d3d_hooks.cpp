...
// Frame publication and live counters share this lock.  A draw hook must not
// increment the counters for frame N and append its record after the boundary
// has already published frame N.  Keeping the short counter/record critical
// sections on the same mutex gives every snapshot one coherent epoch.
template <typename T>
void IncrementLiveStat(std::atomic<T>& counter, T amount = T{1}) {
...
// Caller must hold g_shadow_mutex for at least shared access.
ac6::d3d::ShadowState SnapshotShadowStateUnlocked(uint8_t* base, uint32_t device) {
...
ac6::d3d::ShadowState SnapshotShadowState(uint8_t* base, uint32_t device) {
    ...
    return SnapshotShadowStateUnlocked(base, device);
}
...
void CaptureDrawCall(uint8_t* base, ac6::d3d::DrawCallKind kind, uint32_t device,
                     ...
                     uint32_t flags) {
    const bool capture_enabled = REXCVAR_GET(ac6_render_capture);
    if (capture_enabled) {
        // All state setters use shadow -> capture. Keep the same order here
        // and at the frame boundary to avoid a cross-thread lock inversion.
        std::shared_lock<std::shared_mutex> shadow_lock(g_shadow_mutex);
        ...
        switch (kind) {
            case ac6::d3d::DrawCallKind::kIndexed:
                ...
                g_live_stats.total_indices.fetch_add(count, std::memory_order_relaxed);
                break;
            case ac6::d3d::DrawCallKind::kIndexedShared:
            ...
            case ac6::d3d::DrawCallKind::kPrimitive:
                ...
                g_live_stats.total_vertices.fetch_add(count, std::memory_order_relaxed);
                break;
        }
        ...
        record.count = count;
        record.flags = flags;
        record.shadow_state = SnapshotShadowStateUnlocked(base, device);
...
PPC_FUNC_IMPL(rex_sub_821DD258) {
    ...
    IncrementLiveStat(g_live_stats.set_render_target_calls);
...
PPC_FUNC_IMPL(rex_sub_821DD5C8) {
    ...
    IncrementLiveStat(g_live_stats.set_depth_stencil_calls);
...
PPC_FUNC_IMPL(rex_sub_821DCF28) {
    ...
    IncrementLiveStat(g_live_stats.set_viewport_calls);
...
PPC_FUNC_IMPL(rex_sub_821DC538) {
    ...
    IncrementLiveStat(g_live_stats.set_sampler_state_calls);
...
PPC_FUNC_IMPL(rex_sub_821E10C8) {
    ...
    IncrementLiveStat(g_live_stats.set_texture_fetch_calls);
...
void OnFrameBoundary() {
    // Publish counters and captured records under one epoch.  Every live
    // counter update and record append takes this lock, so the boundary cannot
    // observe a draw in the counters for frame N while its record lands in
    // frame N+1.
    std::shared_lock<std::shared_mutex> shadow_lock(g_shadow_mutex);
    std::unique_lock<std::shared_mutex> capture_lock(g_capture_mutex);
    ...
    frame_capture.frame_end_shadow = SnapshotShadowStateUnlocked(nullptr, 0);
