...
constexpr uint32_t kMaxPacEntryBytes = 256u * 1024u * 1024u;
...
int MemRead(mspack_file* f, void* buf, int bytes) {
    ...
    if (!h || !h->in || bytes < 0 || h->in->pos > h->in->size) return -1;
...
int MemWrite(mspack_file* f, void* buf, int bytes) {
    ...
    if (count > h->out->max_size || h->out->bytes.size() > h->out->max_size - count) return -1;
...
int MemSeek(mspack_file* f, off_t offset, int mode) {
    ...
    if (offset >= 0 && size_t(offset) > h->in->size - base) return -1;
...
bool TryLzxOnce(const uint8_t* compressed, uint32_t csize, uint32_t usize,
    ...
    if (!compressed || !out || csize == 0 || usize == 0 || usize > kMaxPacEntryBytes) {
        return false;
    }
    MemInput in{compressed, csize, 0};
    MemOutput dst;
    dst.max_size = usize;
    dst.bytes.reserve(usize);
    MemFile in_file{};
    MemFile out_file{};
    in_file.in = &in;
    out_file.out = &dst;
...
bool WriteBlob(const std::filesystem::path& path, const uint8_t* data, size_t size) {
    ...
    if (!file) {
        REXFS_ERROR("[AC6 PAC] failed to open temporary dump {}", temp_path.string());
        return false;
    }
    ...
    if (!file) {
        ...
        REXFS_ERROR("[AC6 PAC] failed to write dump {}", temp_path.string());
        return false;
    }
    file.close();
    if (!file) {
        std::filesystem::remove(temp_path, ec);
        REXFS_ERROR("[AC6 PAC] failed to close dump {}", temp_path.string());
        return false;
    }
    std::filesystem::rename(temp_path, path, ec);
    if (ec) {
        // POSIX rename replaces an existing destination; Windows does not.
        // Retry only after the complete temporary file exists.
    ...
    if (ec) {
        ...
        REXFS_ERROR("[AC6 PAC] failed to publish dump {}: {}", path.string(),
...
constexpr size_t kMaxRetainedChunkBytes = 64u * 1024u * 1024u;
...
bool IsRangeCovered(const std::map<uint32_t, ChunkRec>& chunks, uint64_t start, uint64_t end) {
    if (end <= start) return false;
    if (start > UINT32_MAX) return false;
    auto it = chunks.upper_bound(static_cast<uint32_t>(start));
    if (it == chunks.begin()) return false;
    --it;  // greatest chunk_offset <= start
    uint64_t cursor = start;
    while (cursor < end) {
        const uint64_t chunk_off = it->first;
        const uint64_t chunk_end = chunk_off + it->second.bytes.size();
...
std::vector<uint8_t> GatherRange(const std::map<uint32_t, ChunkRec>& chunks, uint64_t start,
                                 uint64_t end) {
    std::vector<uint8_t> out;
    if (end <= start) return out;
    if (start > UINT32_MAX || end - start > SIZE_MAX) return out;
    ...
    while (cursor < end) {
        const uint64_t chunk_off = it->first;
        const uint64_t chunk_len = it->second.bytes.size();
        if (chunk_off > cursor || chunk_off + chunk_len <= cursor) return {};
        const uint64_t local = cursor - chunk_off;
        const uint64_t take = std::min<uint64_t>(chunk_len - local, end - cursor);
        if (local > it->second.bytes.size() || take > it->second.bytes.size() - local) return {};
        const auto* begin = it->second.bytes.data() + static_cast<size_t>(local);
        out.insert(out.end(), begin, begin + static_cast<size_t>(take));
        cursor += take;
        if (cursor >= end) break;
        ++it;
...
void TryDumpEntry(bool is_data01, uint32_t entry_index, ArchiveState* state) {
    ...
    if (rec->compressed_size > kMaxRetainedChunkBytes ||
        rec->decompressed_size > kMaxPacEntryBytes) {
        REXFS_WARN("[AC6 PAC] refusing oversized entry: index={} csize=0x{:x} usize=0x{:x}",
                   rec->index, rec->compressed_size, rec->decompressed_size);
        state->dumped.insert(entry_index);
        return;
    }

    const uint64_t start = rec->offset;
    if (start > UINT64_C(0x100000000) ||
        uint64_t(rec->compressed_size) > UINT64_C(0x100000000) - start) {
        REXFS_WARN("[AC6 PAC] refusing wrapped entry range: index={} offset=0x{:x} csize=0x{:x}",
                   rec->index, rec->offset, rec->compressed_size);
        state->dumped.insert(entry_index);
        return;
    }
    const uint64_t end = start + uint64_t(rec->compressed_size);
...

    {
        ...
        if (!WriteBlob(out_path, raw.data(), raw.size())) return;
    }
...
void Ac6DumpPacDecodedEntry(uint16_t entry_index, uint8_t codec_mode, uint32_t compressed_size,
    ...
    if (!DumpingEnabled() || !host_data || decompressed_size == 0 ||
        decompressed_size > kMaxPacEntryBytes) {
        if (decompressed_size > kMaxPacEntryBytes) {
            REXFS_WARN("[AC6 PAC] refusing oversized decoded entry: index={} usize=0x{:x}",
                       entry_index, decompressed_size);
        }
        return;
    }
...
std::vector<uint8_t> Ac6PeekCompressedHead(uint32_t entry_index, std::size_t max_bytes) {
    ...
    const uint64_t want = std::min<uint64_t>(
        std::min<uint64_t>(max_bytes, rec->compressed_size), kMaxRetainedChunkBytes);
    const uint64_t start = rec->offset;
    if (want == 0 || start + want > UINT64_C(0x100000000)) return {};
    const uint64_t end = start + want;
...
void Ac6OnPacReadCompleted(std::string_view path, uint32_t guest_buffer, uint64_t file_offset,
    ...
    if (file_offset > UINT32_MAX || bytes_read > kMaxRetainedChunkBytes) return;
    if (file_offset > UINT64_MAX - uint64_t(bytes_read)) return;
    const uint64_t range_end = file_offset + uint64_t(bytes_read);
    if (range_end > UINT64_C(0x100000000)) return;

    const uint32_t offset_u32 = static_cast<uint32_t>(file_offset);
    const uint8_t* host = TranslateGuestBuffer(guest_buffer, bytes_read);
    if (!host) return;
    std::vector<uint8_t> captured(host, host + bytes_read);

    std::scoped_lock lock(ArchiveMutex());
    auto& archive = GetArchive(is_data01);
    if (auto existing = archive.chunks.find(offset_u32); existing != archive.chunks.end()) {
        archive.retained_bytes -= existing->second.bytes.size();
        archive.chunks.erase(existing);
    }
    while (archive.retained_bytes + captured.size() > kMaxRetainedChunkBytes &&
           !archive.chunks.empty()) {
        auto oldest = archive.chunks.begin();
        archive.retained_bytes -= oldest->second.bytes.size();
        archive.chunks.erase(oldest);
    }
    archive.retained_bytes += captured.size();
    archive.chunks.emplace(offset_u32, ChunkRec{std::move(captured)});

    const auto candidates = ac6_pac_index::FindOverlapping(is_data01, offset_u32, range_end);
