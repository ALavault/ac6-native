...
SamplerFilterSummary SummarizeSamplerFiltering(
  ...
  for (const auto& sampler : samplers) {
    ...
    if (sampler.mip_min_level != 0 || sampler.mip_max_level != 0) {
      ++summary.mip_clamp_count;
      summary.max_mip_level = std::max(summary.max_mip_level, sampler.mip_max_level);
    }
  }
  return summary;
}
...
RenderEventSignature BuildRenderEventSignature(
  ...
  constexpr uint64_t kFnvOffsetBasis = 1469598103934665603ull;
  // Setter hooks do not cover AC6's inlined resource binds. Prefer the
  // pointer state refreshed from guest device memory at the last captured
  // draw; retain the frame-end shadow as a deterministic fallback for empty
  // or disabled captures.
  const ac6::d3d::ShadowState& effective_shadow = frame_capture.draws.empty()
      ? shadow_state
      : frame_capture.draws.back().shadow_state;
  ...
  signature.render_target_0 = effective_shadow.render_targets[0];
  signature.depth_stencil = effective_shadow.depth_stencil;
  signature.viewport_width = effective_shadow.viewport.width;
  signature.viewport_height = effective_shadow.viewport.height;
  ...
  signature.texture_count = CountNonZero(effective_shadow.textures);
  signature.sampler_count = CountBoundSamplers(effective_shadow.samplers);
  const SamplerFilterSummary sampler_filters =
      SummarizeSamplerFiltering(effective_shadow.samplers);
  ...
  signature.stream_count = CountBoundStreams(effective_shadow.streams);
  signature.vertex_declaration = effective_shadow.vertex_declaration;
  signature.index_buffer = effective_shadow.index_buffer;
  signature.guest_vertex_shader = effective_shadow.guest_vertex_shader;
  signature.guest_pixel_shader = effective_shadow.guest_pixel_shader;
  signature.pixel_shader_program = effective_shadow.pixel_shader_program;
  signature.vertex_shader_program_candidates =
      effective_shadow.vertex_shader_program_candidates;
  signature.fetch_constant_count = CountNonZero(effective_shadow.texture_fetch_ptrs);
  signature.shader_gpr_alloc = effective_shadow.shader_gpr_alloc;
  signature.active_vertex_shader_hash = active_vertex_shader_hash;
  signature.active_pixel_shader_hash = active_pixel_shader_hash;
  signature.has_depth_stencil = effective_shadow.depth_stencil != 0;
  signature.has_resolve = capture_summary.resolve_count != 0;
  signature.half_res_like = IsHalfResLike(effective_shadow, swap_submission);
  signature.quarter_res_like = IsQuarterResLike(effective_shadow, swap_submission);
  signature.post_process_like =
      signature.has_resolve && !signature.has_depth_stencil;
  signature.ui_like =
      IsLikelyUiPass(capture_summary, effective_shadow, swap_submission);
  signature.particle_like =
      IsLikelyParticlePass(capture_summary, effective_shadow);
  signature.point_sprite_like =
      IsLikelyPointSpritePass(capture_summary, effective_shadow);
  ...
  signature.additive_like =
      IsLikelyAdditive(capture_summary, effective_shadow);
  ...
  HashU32(hash, signature.vertex_declaration);
  HashU32(hash, signature.index_buffer);
  HashU32(hash, signature.guest_vertex_shader);
  HashU32(hash, signature.guest_pixel_shader);
  HashU32(hash, signature.pixel_shader_program.guest_address);
  HashU32(hash, signature.pixel_shader_program.size_bytes);
  for (const auto& program : signature.vertex_shader_program_candidates) {
    HashU32(hash, program.guest_address);
    HashU32(hash, program.size_bytes);
  }
  HashU32(hash, signature.fetch_constant_count);
  HashU32(hash, signature.shader_gpr_alloc);
  HashU64(hash, effective_shadow.vertex_fetch_layout_signature);
  HashU64(hash, effective_shadow.texture_fetch_layout_signature);
  HashU64(hash, effective_shadow.resource_binding_signature);
