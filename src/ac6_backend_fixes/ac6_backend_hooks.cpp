...
BackendDrawTelemetry g_draw_telemetry;
...
void ReportHostIssueCalled(const BackendDrawApi api) noexcept {
  g_draw_telemetry.ReportHostIssueCalled(api);
}

bool ReportBackendIssueResult(const BackendDrawApi api, const bool success) noexcept {
  g_draw_telemetry.ReportBackendResult(api, success);
  return success;
}

void ReportHostDrawEmitted(const BackendDrawApi api) noexcept {
  g_draw_telemetry.ReportHostDrawEmitted(api);
}

BackendDrawTelemetrySnapshot GetBackendDrawTelemetrySnapshot() noexcept {
  return g_draw_telemetry.Snapshot();
}
...
void AnalyzeFrameBoundary(
  ...
  g_snapshot.capture_material_identity_draw_count =
      capture_summary.material_identity_draw_count;
  g_snapshot.capture_clear_count = capture_summary.clear_count;
  g_snapshot.capture_resolve_count = capture_summary.resolve_count;
  const BackendDrawTelemetrySnapshot current_draw_telemetry =
      g_draw_telemetry.Snapshot();
  g_snapshot.backend_draw_frame_delta = BackendDrawTelemetryDelta(
      current_draw_telemetry, g_snapshot.backend_draw_telemetry);
  g_snapshot.backend_draw_telemetry = current_draw_telemetry;
