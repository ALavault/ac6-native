...
struct BackendDiagnosticsSnapshot {
  ...
  uint32_t capture_material_identity_draw_count = 0;
  uint32_t capture_clear_count = 0;
  uint32_t capture_resolve_count = 0;
  uint32_t repeated_signature_count = 0;
  BackendDrawTelemetrySnapshot backend_draw_telemetry{};
  BackendDrawTelemetrySnapshot backend_draw_frame_delta{};
...
bool ReportBackendIssueResult(BackendDrawApi api, bool success) noexcept;
