...
void ApplyAc6DefaultSettings() {
    ...
    if (!rex::cvar::HasNonDefaultValue("video_mode_width")) {
        REXCVAR_SET(video_mode_width, ac6::display::kWidth);
    }
    if (!rex::cvar::HasNonDefaultValue("video_mode_height")) {
        REXCVAR_SET(video_mode_height, ac6::display::kHeight);
    }
    if (!rex::cvar::HasNonDefaultValue("resolution")) {
        REXCVAR_SET(resolution, std::string(ac6::display::kResolutionPreset));
    }
    if (!rex::cvar::HasNonDefaultValue("window_width")) {
        REXCVAR_SET(window_width, ac6::display::kWidth);
    }
    if (!rex::cvar::HasNonDefaultValue("window_height")) {
        REXCVAR_SET(window_height, ac6::display::kHeight);
    }
}
...
std::unique_ptr<rex::ui::WindowedApp> Ac6recompAppCreate(rex::ui::WindowedAppContext& ctx) {
    ...
    // Keep the standalone default, but preserve a harness/user-provided path.
    // Concurrent evidence runs must never share and overwrite one log file.
