...
REXCVAR_DEFINE_BOOL(ac6_show_diagnostics_overlay, false, "AC6/NativeGraphics",
                    "Show the AC6 graphics diagnostics panel. Off by default: it covers the left "
                    "40% of the frame, including the left-hand button of a two-option dialog.");
...
void SyncRuntimeFlags(NativeGraphicsRuntimeStatus& status) {
  status.enabled = REXCVAR_GET(ac6_native_graphics_enabled);
  status.mode = ParseGraphicsMode(REXCVAR_GET(ac6_graphics_mode));
  status.capture_enabled = REXCVAR_GET(ac6_render_capture);
  ...
  status.draw_resolution_scale_x =
  ...
  status.direct_host_resolve = REXCVAR_GET(direct_host_resolve);
  status.draw_resolution_scaled_texture_offsets =
      REXCVAR_GET(draw_resolution_scaled_texture_offsets);
  status.authoritative_renderer_active =
      status.enabled && status.mode != GraphicsRuntimeMode::kDisabled;
  if (status.authoritative_renderer_name.empty()) {
    status.authoritative_renderer_name = REXCVAR_GET(ac6_graphics_backend);
  }
}
...
void OnFrameBoundary(rex::memory::Memory* memory) {
  ...
  bool enabled = false;
  GraphicsRuntimeMode mode = GraphicsRuntimeMode::kDisabled;
  {
    std::scoped_lock lock(g_runtime_status_mutex);
    SyncRuntimeFlags(g_runtime_status);
    enabled = g_runtime_status.enabled;
    mode = g_runtime_status.mode;
  }
  g_captured_memory.store(memory, std::memory_order_release);

  if (!enabled || mode == GraphicsRuntimeMode::kDisabled) {
  ...
  const auto backend_diagnostics = ac6::backend::GetDiagnosticsSnapshot();
  std::scoped_lock lock(g_runtime_status_mutex);
  ...
  if (ts && ts->context() && ts->context()->kernel_state) {
    if (auto* concrete_graphics = dynamic_cast<rex::graphics::GraphicsSystem*>(
            ts->context()->kernel_state->graphics_system())) {
      g_runtime_status.authoritative_renderer_name = concrete_graphics->name();
    }
  }
  g_runtime_status.backend_diagnostics = backend_diagnostics;
}
