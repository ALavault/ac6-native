...
void NativeGraphicsStatusDialog::OnDraw(ImGuiIO& io) {
  if (ac6::dialog_text::ShouldDrawFallback()) {
    ImDrawList* draw_list = ImGui::GetForegroundDrawList();
    ImFont* font = ImGui::GetFont();
    const auto layout = ac6::dialog_text::LayoutForHeight(io.DisplaySize.y);
    constexpr ImU32 color = IM_COL32(220, 220, 214, 255);
    ...
    for (size_t i = 0; i < std::size(lines); ++i) {
      const ImVec2 extent =
          font->CalcTextSizeA(layout.font_size, FLT_MAX, 0.0f, lines[i]);
      const float x = (io.DisplaySize.x - extent.x) * 0.5f;
      draw_list->AddText(
          font, layout.font_size,
          ImVec2(x, layout.first_line_y + float(i) * layout.line_spacing),
          color, lines[i]);
    }
  }
  ...
  ImGui::Text("backend total issue / success / host draw: %llu / %llu / %llu",
              static_cast<unsigned long long>(
                  diagnostics.backend_draw_telemetry.host_issue_called),
              static_cast<unsigned long long>(
                  diagnostics.backend_draw_telemetry.backend_success),
              static_cast<unsigned long long>(
                  diagnostics.backend_draw_telemetry.host_draw_emitted));
  ImGui::Text("frame guest MATE / backend issue / success / host draw: %u / %llu / %llu / %llu",
              diagnostics.capture_material_identity_draw_count,
              static_cast<unsigned long long>(
                  diagnostics.backend_draw_frame_delta.host_issue_called),
              static_cast<unsigned long long>(
                  diagnostics.backend_draw_frame_delta.backend_success),
              static_cast<unsigned long long>(
                  diagnostics.backend_draw_frame_delta.host_draw_emitted));
