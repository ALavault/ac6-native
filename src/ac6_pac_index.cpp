...
constexpr uint64_t kMaxPacAddress = UINT64_C(0x100000000);
...
struct PacKey {
    ...
    friend bool operator==(const PacKey&, const PacKey&) = default;
};

struct PacKeyHash {
    size_t operator()(const PacKey& key) const noexcept {
        uint64_t hash = key.is_data01 ? 0x9E3779B97F4A7C15ull : 0x243F6A8885A308D3ull;
        hash ^= uint64_t(key.offset) + 0x9E3779B97F4A7C15ull + (hash << 6) + (hash >> 2);
        hash ^= uint64_t(key.compressed_size) + 0x9E3779B97F4A7C15ull + (hash << 6) + (hash >> 2);
...
struct State {
    ...
    std::unordered_map<PacKey, uint32_t, PacKeyHash> by_offset_csize;
};
...
PacKey MakeKey(bool is_data01, uint32_t offset, uint32_t csize) {
    return PacKey{is_data01, offset, csize};
}
...
bool LoadFromBuffer(const uint8_t* data, size_t size) {
    ...
    std::unordered_map<PacKey, uint32_t, PacKeyHash> by_key;
    ...
    for (uint32_t i = 0; i < count; ++i) {
        ...
        if (uint64_t(e.offset) + uint64_t(e.compressed_size) > kMaxPacAddress) {
            return false;
        }
...
std::vector<uint32_t> FindOverlapping(bool is_data01, uint64_t range_begin, uint64_t range_end) {
    ...
    for (const auto& e : s.entries) {
        ...
        const uint64_t e_begin = e.offset;
        const uint64_t e_end = e_begin + uint64_t(e.compressed_size);
